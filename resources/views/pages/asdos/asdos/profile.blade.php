@extends('layouts.admin')

@section('content')
  
  <!-- Page Heading -->
          <div class="d-sm-flex align-items-center justify-content-between mb-4 bg-primary p-4">
            <h1 class="h3 mb-0 text-light">Profile Asdos</h1>
          </div>

          <!-- Content Row -->
          <div class="container">
          <div class="row">
      
            <div class="card-body text-gray-800">
                <div class="row">
                <div class="col-lg-6">
                <div class="card">
                    <div class="card-body p-3">
                     <img style="border-radius:100px; padding-left:60px;" class="image-fluid" src="data:image/png;base64," alt="">
                     <div class="ml-4 p-4">
                <ul>
                
                <h4 class="text-bold">Asdos</h4>
                @foreach(Auth::user()->matkul as $matkul)
                             <li class="text-bold " style="list-style:none;font-size:20px"> {{$matkul->nama}}</li>   
                 @endforeach
                
                </ul>
                 <br>
                <button class="btn btn-primary btn-md ml-5">Edit Profile</button>
                
                </div>
                </div>  
           
                    
                    </div>
                </div>
                
                 
                <div class="col-1g-6">
                <div class="card">
                    <h5 class="card-header bg-primary text-white">Asdos Profile</h5>
                    <div class="card-body p-3">
                        @foreach($asdos as $item)
                            <h3>Hello My Name is {{Auth::user()->name}}</h3>
                           
                            <p class="card-text">Kode : {{$item->kode}}</p>
                            <p class="card-text">Tempat,Tanggal Lahir :{{ucfirst(strtolower($item->birtday_place))}} , {{date('D -M- Y',strtotime($item->birtday))}}</p>
                            <p class="card-text">Angkatan : {{$item->angkatan}}</p>
                            <p class="card-text">Username ELEN : {{$item->username_elen}}</p>
                            
                            <p class="card-text">{{$item->bank}}</p>
                            <p class="card-text">No Rekening : {{$item->norek}} a/n {{$item->atasnama}}</p>
                           
                         @endforeach
           
                    
                    </div>
                </div>
                
                </div>
           
                </div>
            </div>
          </div>
         
          </div>
          
@endsection